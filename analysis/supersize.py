...
def supersize(df, agent=None):
    g = sns.catplot('after', 'choose_suggested',
        col='n_feature',
        data=df,
        height=4, palette={0: 'C2', 1: '#AED1B2'},
    ...
    plot_chance(g, [1/6])
    # g.set_titles(template='5+1 Options – {col_name} Features')
    g.set_titles(template='{col_name} Features')
    g.set(ylim=(0, 0.5), yticks=(0, 0.5))

plot_both(supersize, data)

# %% ==================== Lines ====================

def suggestion(df, agent=None):
